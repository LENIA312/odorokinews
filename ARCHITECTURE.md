...
| founded_year | INTEGER? | 0004 | 創業年。**世界暦(world.current_date)の年を超える値は保存時に自動でクランプされる**
  （管理画面のPOST/PUT両ルート）。種データが現実世界の年表記(1958〜2011年)のまま投入されており、
  1900年始まりの世界暦から見ると軒並み「未来に創業した会社」になっていた不具合を修正した際に、
  再発防止として追加したガード（`scripts/fix_future_founded_years.sql`で既存値も1800年代後半へ
  補正済み） |
...
道の描画（`roadNetwork()`, `map.ts`）は三次ベジェで軽くうねらせている。**湾曲の大きさは
経路長に比例させ、上限も220pxまで引き上げた**（以前は最大28px固定だったため、都市間をまたぐ
数千px級の長い道では相対的にほぼ真っ直ぐにしか見えず「道が直線すぎる」という不具合になって
いた）。短い道は控えめに、長い道ほど自然にうねる。
...
**地形の装飾**（川・丘・森・湿地・海岸線、`terrain()`）は、もともと創業時からの「旧市街」
（`NATURAL_BOUNDS`、x:0〜1400 y:0〜900）1箇所に手描きで固定配置されたものだった。都市が
遠くに増えるたびに表示範囲(`bounds`)だけがどんどん広がっていき、地形はその小さな一角にしか
無い状態になって「地形も一部しかなくて見ごたえがない」という不具合になっていた。これを補うため
`scatterTerrain(bounds)`を追加し、旧市街の外側を260px四方の格子で走査、各セルに座標由来の
決定論的な疑似乱数（`hash2()`。Math.randomではなく座標のsin値を使うハッシュにすることで、
同じ`bounds`なら毎回同じ地形になり、リロードのたびに地形がちらつくことがない）で森・丘・草地・
何も置かない空き地のいずれかを割り当てる。旧市街の手描き地形はそのまま残し、拡張された範囲側は
この自動散布で薄く埋める形になっている。
...
- 2026-08-25: 企業の創業年(founded_year)が世界暦(1900年始まり)より未来になっていた不具合を
  修正。種データが現実世界の年表記(1958〜2011年)のまま投入されていたため、既存値を1800年代
  後半へ補正（`scripts/fix_future_founded_years.sql`）し、管理画面からの保存時にも世界暦の年を
  超える値を自動でクランプするガードを追加（POST/PUT両ルート）。マップの道が長距離になるほど
  相対的に真っ直ぐ見えていた不具合（湾曲が最大28px固定だったため）を修正し、経路長に比例した
  三次ベジェのうねりに変更（上限220px）。地図の表示範囲が広がるたびに手描きの地形（川・丘・森）
  が旧市街の一角にしか無く見劣りしていた問題を修正し、拡張範囲を格子状に走査して決定論的な
  疑似乱数で森・丘・草地を自動的に薄く散らす`scatterTerrain()`を追加。
